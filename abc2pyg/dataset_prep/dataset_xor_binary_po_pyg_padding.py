...
class XORBinaryPoPaddingDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, highest_order = 16):
        self.highest_order = highest_order
        super(XORBinaryPoPaddingDataset, self).__init__(root, transform, pre_transform, highest_order)
    ...
    def processed_file_names(self):
        return ['data_xor_binary_po_padding.pt']
    ...
    def find_max_num_nodes(self):
        ...
        for data_name in tqdm(root_folders):
            if 'processed' in data_name:
                ...
                folder = os.path.join(self.root, data_name)
                mas_xor_binary_path = os.path.join(folder, 'Mas'+str(self.highest_order)+'.xor_binary_po')
                edge_index = []
