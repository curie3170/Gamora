...
class XORBinaryPaddingDataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, highest_order = 16):
        self.highest_order = highest_order
        super(XORBinaryPaddingDataset, self).__init__(root, transform, pre_transform, highest_order)
    ...
    def processed_file_names(self):
        return ['data_xor_binary_padding.pt']
    ...
    def find_max_num_nodes(self):
        ...
        for data_name in tqdm(root_folders):
            if 'processed' in data_name:
                ...
                folder = os.path.join(self.root, data_name)
                mas_xor_binary_path = os.path.join(folder, 'Mas'+str(self.highest_order)+'.xor_binary')
                edge_index = []
...
if __name__ == '__main__':
    dataset = XORBinaryPaddingDataset(root = '/home/curie/masGen/DataGen/dataset8', highest_order = 8) #, transform=T.ToSparseTensor())
    print(dataset[0].po)
    print(dataset[1])
    print(dataset[2])
    print(dataset[3])
